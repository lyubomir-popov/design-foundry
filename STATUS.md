...
- Cold-start / portability status: this repo now vendors `baseline-foundry` under `vendor/baseline-foundry`, `npm install` refreshes that snapshot via `preinstall`, and `npm run sync:baseline-foundry` rolls it forward from a sibling checkout when available. Fresh clones build without private BF access; the last validated handoff build is `npm run preview:build`. The one shared-shell item still intentionally left upstream is eventual removal of the legacy `baseline-foundry/presets/panel.css` dependency once a fuller worksurface contract exists.
...
- The shell uses the canonical `baseline-foundry` dark application, overlay, and resize contracts. The old local shell class layer is gone from source. The preview now imports the exported `baseline-foundry/presets/panel.css` preset again, so the control rail is back on the shared zero-radius / tighter-padding panel tokens instead of the roomier app-tier preset.
