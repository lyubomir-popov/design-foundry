...
- Shared graph port metadata and validation now live in `packages/document-model/src/background-graph.ts`. Persisted graphs normalize against real input and output ports, reject duplicate or cyclic connections, and keep the graph model reusable across future authoring surfaces.
...
- The operator preset system is now generic: `OperatorPresetController` serves any operator via `getUserPresetDefinitions(operatorKey)` and `saveCurrentPreset(operatorKey, ...)`. A shared `buildOperatorPresetPicker()` builder provides the select + apply + save UI. All four background operators (Halo, Fuzzy Boids, Scatter, Phyllotaxis) have preset pickers in Parameters with built-in presets and file-backed user save. Per-format config storage stays Halo-only.
- Kernel packages (`graph-runtime`, `layout-grid`, `layout-text`) now have 56 Vitest unit tests.
...
- The preview shell now has a Houdini-style Tab-key Add Node menu: pressing Tab (when no form element is focused) opens a centered floating popup listing all available background operators (Phyllotaxis, Scatter, Fuzzy Boids, Halo Field). Clicking or pressing Enter adds the node, selects it, and opens its parameter section. Escape dismisses the popup. Arrow keys navigate between items.
- The config-editor controller no longer carries dead `getAvailableBackgroundOperatorKeys` or `addBackgroundNode` deps, the unused `getBackgroundOperatorLabel` function, or the stale `OVERLAY_SCENE_FAMILY_ORDER` import. Node addition is now exclusively via the shell-level Tab menu.
- Stage 1.5 (format-variant authoring refinement) is complete. Next up is Stage 2 (operatorized scene building), starting with typed `PointField` graph payloads.
...
- Demo rehearsal is closed. The dev preview successfully opened `video-intro-export.brand-layout-ops.json` from the hash route, the in-panel Formats flow updated the stage live for recording, and save/reload preserved the expected active format.
...
- `contentFormatKey` runtime routing has been consolidated: `PreviewState` no longer carries `contentFormatKey` or `contentFormatKeyByDocumentFormatId`, `switchContentFormat()` is removed, and all bucket lookups use the `DEFAULT_CONTENT_FORMAT_KEY` constant. The persistence shape still writes `contentFormatKey` for backward compatibility with older document files.
- `docs/architecture.md` now describes the actual package layer, controller structure, adapter/renderer split, document model types, and export backends instead of aspirational layer sketches.

## Active queue

Lane P is active. Lane R is complete. May 13 demo rehearsal is closed.

- Lane P is active after P7. Copy-on-apply Halo presets, file-backed preset library, panel-hosted Formats, and SVG export are all landed.
- Completed lanes (M, N, O, Q, R) are archived in `HISTORY.md`.
- Future format work follows the hybrid rule: global presets seed authored format variants, variant derivation carries a useful first-guess layout across dimensions, and export presets belong on the future output operator.
- Content-format as a user-facing concept is retired. The document authoring model replaces it. See `ROADMAP.md` for the full synthesis.
...
| Shared document schema and graph persistence | `packages/document-model/src/document-schema.ts` |
| Shared background graph helpers | `packages/document-model/src/background-graph.ts` |
