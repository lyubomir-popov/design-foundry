...
- Lane R is active: overlay text-style edits, grid baseline / safe-area edits, logo lock / size edits, animation fingerprint reuse, and halo label measurement caching are landed. The next work is to remove the remaining controller-driven rebuild-heavy layer-edit and document-target paths, then address controller state and persistence choreography.
