- Lane R is complete: all major interactive paths use localized inspector updates with safe fallback to full rebuild where needed. Operator-pane state is tracked per group with explicit fallback policy. Document-format persistence routes through one shared helper. All remaining `buildConfigEditor()` calls verified as genuinely structural (selection, add/remove, format/family switching). Regression suite covers cross-group state preservation, same-node operation collapse survival, and selected-element-only Parameters scoping. Next frontier is Lane P (format variants) pending P6 decision on same-size variant unlock timing.
# LLM Handoff Context

> Read this first in a new chat.
> Repo just pivoted (2026-05-23). Read `PIVOT.md` BEFORE this file for architecture and reframing.
...
## Post-pivot framing (2026-05-23)

This repo now operates on two distinct tiers:

| Tier | What it is | Status |
|------|-----------|--------|
| **Kernel** (the pivot target) | The Houdini-in-spirit kernel monorepo: typed data graph, incremental DAG runtime, operator interface, flat display-list render IR, harfbuzzjs text-shape WASM, multi-backend renderers. See `PIVOT.md`. | **K1–K7, K9, K10 complete.** 92 verification tests across 8 suites. K9: overlay-preview integration done for all reusable scene families (halo stays on Three.js, K9d deferred). K10: visual parity validated — adapter produces identical style decisions as legacy code, cross-renderer consistency confirmed (SVG + Canvas2D + PDF). K8 deferred (topology settled). Next: K11 (operator migration). |
| **Product surface** (the existing app) | `apps/overlay-preview/` — the Ubuntu Summit / branded-layout authoring app, with halo / boids / phyllotaxis / scatter / spokes scene families, format-variant authoring, and PNG/MP4 export. | **Active and shipping.** Lane R complete, Lane P active. This is the first real kernel consumer and the proving ground. Everything below the "Product surface" subheading in this file describes this tier. |

The two tiers are coupled but progress on different clocks:

- **Kernel work** is greenfield architecture and follows the sequence in `PIVOT.md` (render IR → text-shape WASM → operator surface → first renderer adapter). It will be implemented as new packages alongside the existing ones, NOT by rewriting the overlay-preview app.
- **Product-surface work** continues as before. Lane P, format-variant refinement, halo polish, and export work all proceed on the existing operator-overlay-layout / scene-family pipeline. The eventual migration of overlay-preview onto the kernel is a Stage 1.6+ concern, not blocking near-term product-surface work.

Cold-start agents: if the user's task is product polish / bug-fix / Lane P / Lane R follow-ups / demo work, treat the "Product surface" content below as your working brief. If the user's task is kernel work, read `PIVOT.md` and ignore the lanes.

---

## Product surface — repo boundary

| Role | Path |
|------|------|
| Primary repo | `design-foundry` (current workspace) |
...
- The working artifact is a local `.df.json` file. Browser-local presets are gone from the live workflow.
...
- Overlay text deletion is now keyboard-driven again: selected non-heading text layers are removed with `Delete` instead of a Parameters button, and overlay visibility toggles now force a stage rerender so authored SVG text returns immediately when the overlay is shown again.
...
- The shell uses the canonical `baseline-foundry` dark application, overlay, and resize contracts. The old local shell class layer is gone from source. The preview now imports the supported `baseline-foundry/tiers/os.css` export, while the repo keeps a small local shell-override layer for the top-navigation layout, tighter inspector ergonomics, and authoring-accent chrome on top of BF `os`.
...
- Demo rehearsal is partially closed: the current dev preview successfully opens `video-intro-export.df.json` from the hash route, the in-panel Formats flow updates the stage live for recording, and a disposable `projects/` copy survived save and reload with the expected active format restored from disk. That clears the current browser path, but the actual demo machine's OS-level save/open path still needs to be verified before the final captured slide artifact is considered done.
...
Continue work in this `design-foundry` repo, using a local `racoon-anim` clone as the read-only reference app when parity checks need it. Read `.github/copilot-instructions.md`, `.github/agents/agent.md`, `STATUS.md`, `TODO.md`, `ROADMAP.md`, `docs/specs.md`, and `README.md` first. Treat `TODO.md` as the canonical active queue. Stage 1 parity, the selected-operator pane, the baseline-foundry shell cleanup, the preset-residue cleanup, the graph-first family-persistence migration, Lane J1-J5, and Lane K1-K3 are complete. The current baseline is a `bf-top-navigation` action shell plus a dedicated Layers palette and a layer-following Parameters rail, with persisted documents now storing `sceneFamilyGraphs` without serializing `backgroundGraph`. Lane P is now the active follow-up lane: make the live shell and controller behavior read as authored formats or variants instead of output-only targets, starting with the existing compatible `project.targets` persistence shape.