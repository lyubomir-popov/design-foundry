...
- Lane R is active: overlay text-style edits and style switches, grid baseline / safe-area edits, logo lock / size edits, animation fingerprint reuse, and halo label measurement caching are landed. The next work is to remove the remaining controller-driven rebuild-heavy add/delete, authoring, and document-target paths, then address controller state and persistence choreography.
