...
- Validation: `npm run typecheck`, live preview check confirming a selected text layer's Font Size input stayed connected while a style switch retargeted it from `64` to the clicked `B Head` style's `32`, live preview drag check confirming the existing `Keyline` input stayed connected while the selected `A Head` text moved from keyline `2` to `1`, live preview check confirming the Layout Grid baseline input stayed mounted during edit, live `Fit Safe Area` toggle check confirming the safe-area inputs hide within the section without a full rail rebuild, live logo lock toggle check confirming the checkbox and width input stayed connected while the width-field title updated locally, and live preview render checks after the halo label-metrics cache landed.
